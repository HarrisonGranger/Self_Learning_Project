...
#############################################
# Edited by Harrison Granger
# Edit Time: 2024-3-6 10:39:00
# See line 337, 504.
#############################################
    ...
    def __call__(self):
        if self.val_path is None:
            ...
            if self.dim == 2:
                ...
                plt.scatter()
                plt.bar(self.UMAP_templates[:, 0], self.UMAP_templates[:, 1])
                plt.gca().set_aspect('equal', 'datalim')
                plt.colorbar(boundaries=np.arange(3) - 0.5).set_ticks(np.arange(3))
                plt.title('UMAP_projection_of_{}_{}_{}_training'.format(self.model_name, self.dim, self.SVM_nu))
                self.fig1.savefig(
                    'UMAP_projection_of_{}_{}_{}_training.png'.format(self.model_name, self.dim, self.SVM_nu),
            ...
            if self.dim == 2:
                ...
                self.fig2 = plt.figure(figsize=(10, 8))
                plt.contourf(self.xx, self.yy, self.Z, levels=[0, self.Z.max()], colors='palevioletred')
                plt.scatter(self.UMAP_val[:, 0], self.UMAP_val[:, 1], c=self.val_lables_total,  s=5)
                plt.gca().set_aspect('equal', 'datalim')
                plt.colorbar(boundaries=np.arange(3) - 0.5).set_ticks(np.arange(3))
            ...
            for i in range(len(self.unknown_pre)):
                if self.unknown_pre[i] == 1:
                    shutil.copy(self.unknown.imgs[i][0], './resultant/positive')
                else:
                    shutil.copy(self.unknown.imgs[i][0], './resultant/negative')
            self.val_pre = self.clf.predict(self.UMAP_val)
            self.prediction(self.val_pre, self.val_lables_total)
            self.CM_logger = []
            self.dim_logger = []
            ...
            if self.dim == 2:
                ...
                self.fig1 = plt.figure(figsize=(10, 8))
                plt.contourf(self.xx, self.yy, self.Z, levels=[0, self.Z.max()])
                plt.scatter(self.UMAP_templates[:, 0], self.UMAP_templates[:, 1], c=self.templates_lables_total,
                             s=5)
                plt.gca().set_aspect('equal', 'datalim')
                plt.colorbar(boundaries=np.arange(3) - 0.5).set_ticks(np.arange(3))
                ...
                #########################################################################
                # The first chart we'll add is a bar chart showing the prediction values.
